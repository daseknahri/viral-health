...
OUTPUT — return ONLY a single JSON object, nothing before or after it. No commentary. Use these exact keys. content_html is the ARTICLE BODY ONLY as raw HTML (<p>, <h2>, <h3>, <ul>, <li>, and <strong>/<em> for emphasis — one <strong> per section wrapping a COMPLETE sentence, the section's key takeaway, never a mid-sentence clause; the theme renders <strong> as a standalone bold callout, so a partial fragment renders broken) — no <html>/<head>, no title inside it, no markdown, no code fences inside the string. Do NOT include any URL or <a> anchor anywhere (the site adds the real link automatically; the body stays URL-free). Respect the limits.

{
  ...
  "image_prompt": "a ready-to-paste 4:5 PORTRAIT-MASTER prompt. DEFAULT = SCROLL-STOPPER / growth mode (the Content-to-Thumbnail method in docs/image-strategy.md): MINE THIS article's hook for the single relatable 'caught moment' — a SPECIFIC everyday scene + a BOLD genuine reaction (a real laugh / wide-eyed surprise / transported nostalgia / relieved grin) + the body-gesture that makes the curiosity visible. Punchy, high-contrast, the reacting face the clear focal point, subject centered ~50-60% with a clean soft LOWER-THIRD for the overlay, ONE bold brand accent (sage-green OR muted-burgundy) pinned to a concrete object. Stay on the safe side of the line: everyday amusement/surprise only, NEVER pain/fear/distress/clinical/medical/before-after; no baked text. (Switch to the calm-editorial BRAND mode — image-strategy §4 — once the brand is recognized.) End with exactly: Portrait orientation, 4:5, high resolution. No text, no words, no letters, no logos, no watermark, no numbers. One person only. Natural, undistorted hands and faces, correct number of fingers; no clinical or medical imagery.",
  ...
  "overlay_text": "the facebook_hook compressed to a 5-8 word STANDALONE CURIOSITY FRAGMENT (hard max 9 — longer hooks get truncated on the OG/link card): NOT the title/seo_title restated, NO terminal period, opens the SAME loop as facebook_hook (same withheld specific). Self-check: if removing the title makes it meaningless, or it answers its own question, rewrite it",
  "bottom_hint_text": "LINK IN FIRST COMMENT",
  "discover_image_prompt": "a ready-to-paste DISCOVER-ONLY prompt for a 16:9 LANDSCAPE hero (Google Discover; NEVER fed to the local card converter — full formula in docs/image-strategy.md §5): the same relatable adult + safe action + calm/curious expression, placed slightly OFF-CENTER toward one side within the center two-thirds (nothing important in the top/bottom 12%), opposite side soft low-detail open space; soft warm natural daylight, cozy modern US home, ONE subtle accent (sage-green OR muted-burgundy), 50mm f/2.0 shallow depth of field, realistic skin texture. End with exactly: Landscape orientation, 16:9, high resolution. No text, no words, no letters, no logos, no watermark, no numbers. One person only. Natural, undistorted hands and faces, correct number of fingers; no clinical, medical-procedure, or before/after imagery.",
  "pin_set": [
    { "overlay_text": "on-pin hook, 5-9 words, general framing", "board": "EXACTLY ONE of the canonical boards (no new names): Why Does My Body Do That | Everyday Health Facts | Body Signals | Healthy Habits | Sleep & Energy | Food & Nutrition Facts", "pin_title": "searcher phrasing, under 90 chars, DIFFERENT from the article title and from other pins' first 4 words", "pin_description": "front-load the keyword in the first ~6 words, helpful, under 420 chars, NO URL, end with 2-3 plain hashtags" }
  ],
  ...
  "newsletter_blurb": "a warm 2-4 sentence email blurb of the same general-info payoff that makes a subscriber want the full piece; calm, honest, no fear/clickbait/medical claim/invented number; NO URL (the link is added in the email tool)",
  "x_thread": ["post 1 = the curiosity hook, under 280 chars", "2-3 middle posts delivering the general-info payoff, no fear/clickbait/medical, no invented numbers", "last post = a calm follow CTA; NO URL or link in any post"]
}

The last keys are the **multi-channel pack + renditions** — one topic fanned into platform-native outputs: Pinterest (`pin_set`, 3-5 DISTINCT angles), short-form video (`reel_script`), Google Discover (`discover_image_prompt` + the HONEST `seo_title`), Reddit/Quora (`community_answer`), the OWNED-audience email (`newsletter_blurb`), and X (`x_thread`). **Emit only what you'll actually run** (per `docs/operating-doctrine.md`): `pin_set` + the free `seo_title`/`discover_image_prompt` byproducts are the default; add `reel_script` / `community_answer` / `newsletter_blurb` / `x_thread` only when you're running that surface — don't generate review-surface for channels you aren't posting to. Same non-negotiable guardrails apply to every one (general info only, no diagnosis/personal framing, no invented numbers, NO URLs — you add links by hand when posting).

Before answering, silently verify: valid JSON; content_html and all social fields are URL-free; no diagnosis/cure/advice claims (including in pin_set / reel_script / community_answer); no invented numbers; every explanatory/causal claim is consensus OR explicitly framed as a leading idea / single study / competing theories (never a contested claim stated as settled), and any stated cause connects to any stated fix; `title`, `seo_title`, and `pinterest_title` are three distinct strings and `seo_title` is the honest topic (not the curiosity hook); `overlay_text` is not the title restated and has no terminal period; whichever pack/rendition fields you included are native and URL-free; content_html is 4-6 dense sections with one escalation beat and a specific named second-click cue; every length limit met. Then output ONLY the JSON object.
```
