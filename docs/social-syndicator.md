...
Scheduling applies to your **Facebook Page only** — this is the official Graph API. Group posting stays manual, as before. One Facebook limitation matters: you cannot comment on a post that has not been published yet. The plugin handles this for you: when you schedule a Page post it also schedules a one-shot WP-Cron task that **auto-posts the link first comment about five minutes after the post goes live** (Page only — it never touches groups, and it's idempotent so it won't double-post). WP-Cron fires on site traffic, so if your site is quiet the comment may land a little late; the manual `Post first comment` button in the Social Editor remains as a fallback (and as a retry if an immediate post's comment failed). `Reset Facebook posting lock` cancels a pending auto-comment and lets you reschedule.
...
The `Crop focus` select controls which part of a **tall** source photo the card keeps when it has to crop the height: **keep top** (saves faces/heads), **keep center** (default), or **keep bottom**. It only affects sources that are taller than the card; when the source is wider than the card the full height is already kept, so the setting has no effect there. Pair it with the overlay position — for example, keep the top of the photo and put the hook at the bottom.
