...
- The 18 core bundle keys, plus the multi-channel pack + rendition keys (`discover_image_prompt`, `pin_set`, `reel_script`, `community_answer`, `newsletter_blurb`, `x_thread`) — OPTIONAL, emit only what the active channels use (per `docs/operating-doctrine.md`: `pin_set` + the free `seo_title`/`discover_image_prompt` byproducts by default; add `reel_script`/`community_answer`/`newsletter_blurb`/`x_thread` only when actually running that surface — don't create review-surface for channels you aren't posting to). Every field that IS present is quality-checked in full under D. No other extras. A single JSON object (not an array).
...
- Limits: title <60 · seo_title <58 · meta_description 140–180 · excerpt present · facebook_hook <95 · facebook_summary 180–260 · pinterest_title <90 · pinterest_description <420 · pinterest_alt_text <125 · reddit_title <120 · image_alt <125 · overlay_text ≤9 words (target 5–8; complete fragment — longer hooks truncate on the OG/link card) · bottom_hint_text ≤6 words.
...
- **`pin_set`:** an array of **≤5** pins, each a genuinely DISTINCT searcher intent (rotate ≥3 of: how/why explainer · the-real-reason myth-bust · surprising-detail · practical-tip · relatable-moment). Per pin: `overlay_text` ≤12 words (general framing) · `board` is EXACTLY ONE of the canonical boards, no invented names (*Why Does My Body Do That | Everyday Health Facts | Body Signals | Healthy Habits | Sleep & Energy | Food & Nutrition Facts*) · `pin_title` <90, differs from the article title and from other pins' first 4 words · `pin_description` ≤420, front-loads its keyword in the first ~6 words, ends with 2–3 plain hashtags · **zero URLs in any pin field**.
...
- **`newsletter_blurb`** (the OWNED-audience rendition): a warm 2–4 sentence email blurb of the SAME general-info payoff that makes a subscriber want the full piece — calm, honest, no fear/clickbait/medical-claim/invented-number; **URL-free** (the operator adds the link in the email tool). Note: activate the newsletter on its own first (it needs a compliant email setup — unsubscribe + sender identity).
- **`x_thread`** (an ON-PLATFORM brand rendition): an array of **3–5** short posts, each ≤280 characters. Post 1 = the curiosity hook; the middle posts deliver the general-info payoff (no fear/clickbait/medical, no invented numbers, contested-science hedged); the last = a calm follow CTA. **No URL/link in any post.** Native to X, never a copy of the article body.
