...
- **Multi-channel pack (only the fields you included):** `pin_set` ≤5 pins, each a genuinely distinct angle, every pin field URL-free; `reel_script` hook ≤12 words, beats/voiceover general-only and held to the `overlay_text` banned-phrase bar, `cta` is a follow/join prompt with NO URL, no symptom-as-self-diagnosis; `community_answer` answers GENERALLY ("why most people…"), never "you might have / in your case", and includes the calm "a clinician is the right person for your specific case" note; **Discover title honesty** — `seo_title` states the real topic, never the curiosity `facebook_hook`. (This mirrors section D of `/guardrail-audit`.)
